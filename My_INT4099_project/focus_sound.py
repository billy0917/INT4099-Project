...
import argparse
from playsound import playsound  # 添加音效播放庫
...
class FocusMonitor:
    ...
    def create_default_sound_file(self):
        ...
        """
        print(f"聲音提示文件不存在: {self.alert_sound_path}")
        print("請在以下路徑放置名為'alert.mp3'的聲音文件:")
        print(f"  {self.sound_dir}")
        print("或者修改程式碼中的聲音文件路徑")
    ...
    def detect_focus(self, frame):
        ...
        # 檢查專注狀態變化，觸發聲音提示
        if self.was_focused and not self.is_focused:
            # 從專注變為分心，播放提示音
